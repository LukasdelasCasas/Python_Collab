...
from hw03_perceptron.utils import dot
...
class PerceptronClassifier:
    ...
    def update(self, instance):
        ...
        error = self.weights != predicted_output # TODO: Ex. 7: Replace 0 with the correct calculation of the error
        do_update = error != 0
        ...
        return do_update  # TODO: Ex. 7: Replace pass with update of feature weights
        #sarah: die Gewichte "self.weights" werden dann dementsprechend korrigiert of sie FALSE oder TRUE sein sollen
    ...
    def prediction_f_measure(self, dataset, for_label):
        ...
        """
        ## need to calculate false positives, true positives, false negatives and true negatives
        ...
        trueneg = 0
        for instance in dataset.instance_list:
            if self.prediction(instance.feature_counts) == True and instance.label == True:
                truepos += 1
            if self.prediction(instance.feature_counts) == True and instance.label == False:
                falsepos += 1
            if self.prediction(instance.feature_counts) == False and instance.label == True:
                falseneg += 1
            if self.prediction(instance.feature_counts) == False and instance.label == False:
                trueneg += 1
            return trueneg, falseneg, falsepos, truepos

        if for_label == True:
            precision = truepos/(truepos+falsepos)
            recall = truepos/(truepos+falseneg)

        elif for_label == False:
            precision = trueneg/(trueneg+falseneg)
            recall = trueneg/(trueneg+falsepos)
